...
start:
    ...
    ; some BIOSes might start execution at 07C0:0000 instead of 0000:7C00.
    ; This ensures we are where we expect to be.
    push es
    push word .after
    retf
.after:
    ...
    ; read drive parameters, instead of relying on data from formatted disk
    push es
    mov ah, 08h
    int 13h
    jc floppy_error
    ...
    ; compute LBA on root dir = reserved + fats * sectors_per_fat
    ...
    inc ax                          ; division remainder != 0, add 1
                                    ; this means a partially filled sector
.root_dir_after:

    ; read root directory
    mov cl, al                      ; cl = number of sectors to read = size of root dir
    pop ax                          ; ax = LBA of root dir
    mov dl, [ebr_drive_number]      ; dl = drive number (which we saved earlier)
...
.load_kernel_loop:
    ...
    add ax, 31                      ; first cluster = (cluster number -2) * sectors_per_cluster + start_sector
                                    ; start sector = reserved + fats + rootDirSize = 1 + 18 + 134 = 33
    mov cl, 1
    mov dl, [ebr_drive_number]
    call disk_read

    ; this will probably overflow at some point... should fix it then maybe?
    ...
    mul cx
    mov cx, 2
    div cx                          ; ax = index of entry in FAT, dx = cluster mod2
